...
int main() {
    ...
    if (socket_module != NULL) {
        ...
        if (pFunc && PyCallable_Check(pFunc)) {
            ...
            socket = PyObject_CallObject(pFunc, pArgs);
            if (!socket)
                printf("error socket\n");
        } else 
    ...
    if (transport_module != NULL) {
        ...
        if (pFunc && PyCallable_Check(pFunc)) {
            ...
            //set socket
            PyTuple_SetItem(pArgs, 0, socket);

            transport = PyObject_CallObject(pFunc, pArgs);
            if (!transport)
    ...
    if (protocol_module != NULL) {
        ...
        if (pFunc && PyCallable_Check(pFunc)) {
            ...
            //set transport
            PyTuple_SetItem(pArgs, 0, transport);
            ...
            protocol = PyObject_CallObject(pFunc, pArgs);
            if (!protocol)
    ...
    if (scribe_module != NULL) {
        ...
        //set protocol
        PyTuple_SetItem(pArgs, 0, protocol);
       
        client = PyObject_CallObject(pFunc, pArgs);
        if (!client)
            printf("error client");
    } else 
    ...
    //open transport
    pFunc = PyObject_GetAttrString(transport, "open");
    if (pFunc && PyCallable_Check(pFunc)) {
        pArgs = PyTuple_New(0);
   ...
   //create log entry 
    pFunc = PyObject_GetAttrString(scribe_module, "LogEntry");
    if (pFunc && PyCallable_Check(pFunc)) {
        pArgs = PyTuple_New(2);
    ...
    pFunc = PyObject_GetAttrString(client, "Log");
    if (pFunc && PyCallable_Check(pFunc)) {
        pArgs = PyTuple_New(1);
